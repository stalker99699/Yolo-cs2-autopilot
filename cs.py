...
# ================= НАСТРОЙКИ =================
MODEL_PATH = r"C:\Users\Стас\Documents\CS_YOLO\best.pt"

CONF_THRESHOLD = 0.50
IMG_SIZE = 640
TARGET_FPS = 30
SKIP_FRAMES = 1  # на GPU можна зменшити або поставити 1
...
# GPU налаштування
USE_GPU = True
HALF_PRECISION = True  # FP16 — сильно прискорює на NVIDIA
...
if USE_GPU and torch.cuda.is_available():
    device = 0
    log(f"✅ NVIDIA GPU виявлено: {torch.cuda.get_device_name(0)}")
    log(f"   VRAM: {torch.cuda.get_device_properties(0).total_memory / 1024**3:.1f} GB")
else:
    device = "cpu"
    HALF_PRECISION = False
    log("⚠️ GPU не знайдено, використовуємо CPU")

log("Завантаження моделі...")
try:
    model = YOLO(MODEL_PATH, task="detect")
    # Переносимо модель на GPU одразу
    if device == 0:
        model.to(device)
    log("✅ Модель завантажена.")
except Exception as e:
    log(f"❌ Помилка моделі: {e}")
    sys.exit(1)
...
def capture_loop():
    ...
    log("🚀 Запуск циклу захоплення екрану...")
    
    JSON_PATH = "detections.json"
    with open(JSON_PATH, 'w', encoding='utf-8') as f: 
        json.dump({"timestamp": time.time(), "count": 0, "detections": []}, f)

    with mss.MSS() as sct:
        monitor = sct.monitors[1] if len(sct.monitors) > 1 else sct.monitors[0]
        log(f"🖥 Монітор: {monitor['width']}x{monitor['height']}")
        ...
        while True:
            ...
            try:
                ...
                if frame_count % SKIP_FRAMES == 0:
                    # Головна зміна — використовуємо GPU
                    res = model(img, 
                               conf=CONF_THRESHOLD, 
                               device=device,
                               imgsz=IMG_SIZE, 
                               verbose=False,
                               half=HALF_PRECISION)[0]
                    ...
                    with json_lock:
                        ...
                        with open(JSON_PATH, 'w', encoding='utf-8') as f:
                            json.dump(current_detections.copy(), f, indent=2)
                    
                    demo = img.copy()
                    with frame_lock:
                        latest_frame_bytes = cv2.imencode('.jpg', 
                            draw_overlay(demo, dets), 
...
async def demo():
    ...
    <h3>🎯 YOLO TriggerBot Demo (GPU)</h3>
    <div style="display:flex;gap:10px;">
    <img src="/stream" style="max-width:65vw;border:2px solid #0f0;">
    <pre id="j" style="flex:1;background:#222;padding:10px;overflow:auto;height:85vh;">Loading...</pre>
    </div>
    <script>
    setInterval(async()=>{ 
        document.getElementById('j').textContent = 
            JSON.stringify(await(await fetch('/detections')).json(), null, 2)
    }, 100)
    </script></body></html>"""
